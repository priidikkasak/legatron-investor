...
const WRAP: React.CSSProperties = { maxWidth: 1040, margin: "0 auto", padding: "0 40px" };

/* Section backgrounds alternate OFF / WHITE */
const SEC = (bg: string): React.CSSProperties => ({
  background: bg,
  borderTop: `1px solid ${RULE}`,
  padding: "88px 0",
});

/* Subsection label above tables / lists */
...
/* Standard 2-column content split */
...
function Tag({ n, label }: { n: string; label: string }) {
  return (
    <p style={{ fontSize: 11, fontWeight: 400, letterSpacing: "0.14em", textTransform: "uppercase", color: MUTED, marginBottom: 32 }}>
      {n} — {label}
    </p>
  );
}
...
function H2({ children }: { children: React.ReactNode }) {
  return (
    <h2 style={{ fontSize: "clamp(1.6rem, 2.4vw, 2.1rem)", fontWeight: 500, letterSpacing: "-0.02em", color: INK, lineHeight: 1.2, marginBottom: 32 }}>
      {children}
    </h2>
  );
}
...
function Body({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontSize: "0.9375rem", fontWeight: 300, color: "#333333", lineHeight: 1.85, marginBottom: 16 }}>
      {children}
    </p>
  );
}
...
function List({ items }: { items: string[] }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {items.map((item, i) => (
        <div key={i} style={{ display: "flex", gap: 12, fontSize: "0.875rem", fontWeight: 300, color: "#333333", lineHeight: 1.7 }}>
...
function Box({ label, children, accent }: { label?: string; children: React.ReactNode; accent?: boolean }) {
  return (
    <div style={{
      padding: "24px 28px",
      border: `1px solid ${accent ? "rgba(184,117,10,0.3)" : RULE}`,
      background: accent ? "rgba(184,117,10,0.03)" : WHITE,
      borderRadius: 4,
    }}>
      {label && <p style={{ ...LBL, marginBottom: 12 }}>{label}</p>}
      {children}
    </div>
  );
}
    ...
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr style={{ borderBottom: `1px solid ${INK}` }}>
          {cols.map((c, i) => (
            <th key={i} style={{ ...LBL, marginBottom: 0, textAlign: "left", padding: "0 24px 10px 0", width: i === 0 ? "40%" : undefined }}>
              {c}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {rows.map((row, ri) => (
          <tr key={ri} style={{ borderBottom: `1px solid ${RULE}` }}>
            {row.map((cell, ci) => (
              <td key={ci} style={{ fontSize: "0.875rem", fontWeight: 300, color: "#333333", lineHeight: 1.6, padding: "12px 24px 12px 0", verticalAlign: "top" }}>
                {cell}
              </td>
            ))}
          </tr>
...
function FAQ({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div onClick={() => setOpen(!open)} style={{ borderBottom: `1px solid ${RULE}`, padding: "16px 0", cursor: "pointer" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 24, alignItems: "flex-start" }}>
        <span style={{ fontSize: "0.9rem", fontWeight: 400, color: INK, lineHeight: 1.5 }}>{q}</span>
        <span style={{ color: MUTED, flexShrink: 0, lineHeight: 1, fontSize: "1rem", transition: "transform 0.2s", transform: open ? "rotate(45deg)" : "none", display: "block", marginTop: 2 }}>+</span>
      </div>
      {open && (
        <p style={{ marginTop: 12, fontSize: "0.875rem", fontWeight: 300, color: MUTED, lineHeight: 1.85 }}>{a}</p>
      )}
    </div>
  );
}
...
function Logo({ s = 20 }: { s?: number }) {
  return (
    <svg width={s} height={s} viewBox="0 0 20 20" fill="none">
      <rect x="1" y="1" width="8" height="8" fill={INK} />
      <rect x="11" y="11" width="8" height="8" fill={ACCENT} />
    </svg>
  );
}
...
export default function Page() {
  return (
    <div style={{ background: OFF }}>
      ...
      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, background: "rgba(250,250,248,0.95)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${RULE}` }}>
      ...
      {/* ── HERO ─────────────────────────────── */}
      <section style={{ padding: "136px 0 88px" }}>
        <div style={WRAP}>
          <p style={{ fontSize: 11, fontWeight: 400, letterSpacing: "0.18em", textTransform: "uppercase", color: MUTED, marginBottom: 48 }}>
            Legatron · Strateegiline ariplaan · 2024
          </p>

          <h1 style={{ fontSize: "clamp(3rem, 6vw, 5.2rem)", fontWeight: 500, letterSpacing: "-0.03em", color: INK, lineHeight: 1.05, maxWidth: 800, marginBottom: 32 }}>
            Muudame seadused{" "}
            <span style={{ color: ACCENT }}>infrastruktuuriks.</span>
          </h1>

          <p style={{ fontSize: "1.05rem", fontWeight: 300, color: MUTED, lineHeight: 1.8, maxWidth: 480, marginBottom: 48 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            <a href="#missioon" style={{ display: "inline-block", background: INK, color: OFF, fontSize: "0.8rem", fontWeight: 500, letterSpacing: "0.04em", padding: "10px 22px", borderRadius: 3, textDecoration: "none" }}>
              Vaata ariplaan
            </a>
          ...
          {/* 3 võtmenumbrit */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 40, marginTop: 88, paddingTop: 40, borderTop: `1px solid ${RULE}` }}>
            {[
              ...
              <div key={i}>
                <div style={{ fontSize: "2.6rem", fontWeight: 500, letterSpacing: "-0.03em", color: ACCENT, lineHeight: 1, marginBottom: 8 }}>{s.n}</div>
        ...
        <div style={WRAP}>
          ...
          <div style={SPLIT}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <Box label="Ühe lause missioon" accent>
              ...
              <div>
                <Lbl>Mida kaotaks maailm ilma Legatronita</Lbl>
                <List items={[
      ...
      <section id="lahendus" style={SEC(OFF)}>
        <div style={WRAP}>
          ...
          <Box label="Pohihüpotees - mida teised ei näe" accent>
            <p style={{ fontSize: "0.9375rem", fontWeight: 300, color: "#333333", lineHeight: 1.85 }}>
          ...
          <div style={{ ...SPLIT, marginTop: 48 }}>
            <div>
              <Lbl>Võrdlus - jurist vs Legatron</Lbl>
              <Table
                cols={["", "Jurist", "Legatron"]}
                rows={[
                  ["Hind",           "150-400 € / tund",   <span style={{ color: ACCENT }}>alates 29 € / küsimus</span>],
                  ["Vastusekiirus",  "1-5 tööpäeva",       <span style={{ color: ACCENT }}>sekunditega</span>],
                  ["Kättesaadavus",  "tööajal",            <span style={{ color: ACCENT }}>24/7</span>],
                  ["Kvaliteet",      "kogemus varieerub",  <span style={{ color: ACCENT }}>konsistentne</span>],
                  ["Mastaap",        "ei skaleeru",        <span style={{ color: ACCENT }}>miljonid küsimused</span>],
                ]}
              />
            </div>
            <div>
              <Lbl>Praegune seis</Lbl>
              <Table
                cols={["Valdkond", "Seis"]}
                rows={[
          ...
          <div style={{ marginTop: 48 }}>
            <Lbl>Meid on raske kopeerida - 4 omavahel põimunud komponenti</Lbl>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", border: `1px solid ${RULE}`, borderRadius: 4, overflow: "hidden" }}>
              {[
                ...
                <div key={i} style={{ padding: "24px 20px", background: WHITE, borderLeft: i > 0 ? `1px solid ${RULE}` : "none" }}>
                  <p style={{ fontSize: "1.3rem", fontWeight: 500, color: RULE, letterSpacing: "-0.02em", lineHeight: 1, marginBottom: 12 }}>{m.n}</p>
      ...
      <section id="turg" style={SEC(WHITE)}>
        <div style={WRAP}>
          ...
          <div style={{ ...SPLIT, marginTop: 48 }}>
            ...
            <div>
              <Lbl>Turu laienemine</Lbl>
              <Table
                cols={["Faas", "Fookus"]}
                rows={[
                  [<span style={{ color: ACCENT }}>Faas 1 — 2024</span>,      "Eesti MVP - kiire iteratsioon, digitaalne ühiskond"],
      ...
      <section id="strateegia" style={SEC(OFF)}>
        <div style={WRAP}>
          ...
          <div style={{ marginTop: 48 }}>
            <Box label="#1 Prioriteet sel aastal" accent>
              <p style={{ fontSize: "0.9375rem", fontWeight: 400, color: INK, lineHeight: 1.75 }}>
          ...
          <div style={{ marginTop: 48 }}>
            <Lbl>Peamised kitsaskohad ja lahendused</Lbl>
            <Table
      ...
      <section id="taitmissusteem" style={SEC(WHITE)}>
        <div style={WRAP}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 48 }}>
            <div>
              <Lbl>Kvartalieesmärgid 2024</Lbl>
              <Table
            ...
            <div>
              <Lbl>Igakuised vahe-eesmärgid</Lbl>
              <Table
                cols={["Kuu", "Vahe-eesmärk"]}
                rows={[
                  [<span style={{ color: ACCENT }}>1</span>, "MVP launch, Eesti seadused, esimesed 100 kasutajat"],
            ...
            <div>
              ...
              <Table
                ...
                rows={[
                  ...
                  ["Legal", "Oiguslik täpsus, valideerimisprotsess, riskijuhtimine"],
                ]}
              />
            </div>
          </div>
      ...
      <section id="toode" style={SEC(OFF)}>
        <div style={WRAP}>
          ...
          <Box label="V1 — Launchitav MVP" accent>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 40px" }}>
              {[
                "Kõik Eesti kehtivad seadused",
                "Automaatne uuendus muudatuste korral",
                "Riigikohtu lahendid integreeritud",
                "AI-põhine otsing",
                "Seaduseviited iga vastuse juures",
                "Lihtne keeleline selgitus",
              ].map((f, i) => (
                <span key={i} style={{ fontSize: "0.875rem", fontWeight: 300, color: "#333333", display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ color: ACCENT, fontSize: "0.6rem" }}>◆</span>{f}
                </span>
              ))}
            </div>
          </Box>
      ...
      <section id="hinnad" style={SEC(WHITE)}>
        <div style={WRAP}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 48 }}>
            {[
              {
                sub: "Projektipõhine", seg: "Eraisik",
                price: "€29", unit: "küsimus", featured: false,
                items: ["Üks detailne vastus", "Seaduseviited ja kohtulahendid", "Praktilised järgmised sammud", "Üürileping, tööoigus, tarbijakaitse"],
              },
              {
                sub: "Subscription", seg: "VKE",
                price: "€49", unit: "kuu", featured: true,
                items: ["Piiramatu arv küsimusi", "Kõik oigusvaldkonnad", "Dokumendianalüüs", "Lepingute eelkontroll", "Kuni 50 töötajaga ettevoted"],
              },
              {
                sub: "Ettevote", seg: "Premium",
                price: "€199", unit: "kuu", featured: false,
                items: ["Kõik VKE eelised", "Lepingugeneraator", "Riskianalüüs soovitustega", "Prioriteetne tugi"],
              },
              {
                sub: "B2B Custom", seg: "Enterprise",
                price: "€1k-10k", unit: "kuu", featured: false,
                items: ["API integratsioon", "SLA-garantiid", "Dedikeeritud kontroll", "Rahvusvaheline laienemine", "Pangad, kindlustus, korporatsioonid"],
              },
            ].map((p, i) => (
              <div key={i} style={{ border: `1px solid ${p.featured ? ACCENT : RULE}`, borderRadius: 4, padding: "24px 20px", background: p.featured ? "rgba(184,117,10,0.03)" : OFF, position: "relative" }}>
                {p.featured && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: ACCENT, borderRadius: "4px 4px 0 0" }} />}
                <p style={{ ...LBL, marginBottom: 4 }}>{p.sub}</p>
                <p style={{ fontSize: "0.85rem", fontWeight: 500, color: INK, marginBottom: 16 }}>{p.seg}</p>
                <div style={{ marginBottom: 20 }}>
                  <span style={{ fontSize: "2rem", fontWeight: 500, letterSpacing: "-0.03em", color: p.featured ? ACCENT : INK, lineHeight: 1 }}>{p.price}</span>
                  <span style={{ fontSize: "0.78rem", fontWeight: 300, color: MUTED, marginLeft: 5 }}>/ {p.unit}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8, borderTop: `1px solid ${RULE}`, paddingTop: 16 }}>
                  {p.items.map((f, j) => (
                    <span key={j} style={{ fontSize: "0.78rem", fontWeight: 300, color: MUTED, lineHeight: 1.5 }}>— {f}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          <Lbl>Konkurentsihinnad võrreldes alternatiividega</Lbl>
          <Table
      ...
      <section id="visioon" style={SEC(OFF)}>
        <div style={WRAP}>
          ...
          <div style={SPLIT}>
            ...
            <div>
              <Lbl>Korratav laienemismudel</Lbl>
              <Table
                cols={["Samm", "Tegevus"]}
                rows={[
          ...
          <div style={{ marginTop: 48 }}>
            <Lbl>Kasutusnäited rahvusvahelisel tasandil</Lbl>
            <Table
        ...
        <div style={WRAP}>
          ...
          <div style={SPLIT}>
            ...
            <div>
              ...
              <div style={{ marginTop: 40 }}>
                <Lbl>Strateegilised küsimused</Lbl>
                <div style={{ borderTop: `1px solid ${RULE}` }}>
                  {[
      ...
      {/* ── CTA ──────────────────────────────── */}
      <section style={{ padding: "120px 0", borderTop: `1px solid ${RULE}` }}>
        <div style={{ ...WRAP, textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(2rem, 4.5vw, 3.8rem)", fontWeight: 500, letterSpacing: "-0.03em", color: INK, lineHeight: 1.1, marginBottom: 20, maxWidth: 580, marginLeft: "auto", marginRight: "auto" }}>
            Seadused ei pea olema{" "}
            <span style={{ color: ACCENT }}>keerulised.</span>
          </h2>
          <p style={{ fontSize: "0.9375rem", fontWeight: 300, color: MUTED, marginBottom: 8, lineHeight: 1.7 }}>
            Legatron muudab need arusaadavaks.
          </p>
          <p style={{ fontSize: "0.875rem", fontWeight: 300, color: MUTED, marginBottom: 48 }}>
            We win because we turn law into infrastructure.
          </p>
          <a href="mailto:info@legatron.ai" style={{ display: "inline-block", background: INK, color: OFF, fontSize: "0.85rem", fontWeight: 500, letterSpacing: "0.04em", padding: "12px 28px", borderRadius: 3, textDecoration: "none", marginBottom: 64 }}>
            info@legatron.ai
          </a>
          <div style={{ borderTop: `1px solid ${RULE}`, paddingTop: 28, fontSize: 11, fontWeight: 300, color: "#BBBBBB", letterSpacing: "0.08em" }}>
